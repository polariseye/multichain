...
func (tx *Tx) SignMult(paramList ...interface{}) error {
	...
	for paramPariIndex := 0; paramPariIndex < len(paramList)/2; paramPariIndex++ {
		...
		for _, rsv := range signatures {
			...
			pubKeyScript := tx.inputs[inputIndex].Output.PubKeyScript
			sigScript := tx.inputs[inputIndex].SigScript

			// Support segwit.
			if sigScript == nil {
				if txscript.IsPayToWitnessPubKeyHash(pubKeyScript) || txscript.IsPayToWitnessScriptHash(pubKeyScript) {
					tx.msgTx.TxIn[inputIndex].Witness = wire.TxWitness([][]byte{append(signature.Serialize(), byte(txscript.SigHashAll)), pubKey})
					continue
				}
			} else {
				if txscript.IsPayToWitnessScriptHash(sigScript) || txscript.IsPayToWitnessScriptHash(sigScript) {
					tx.msgTx.TxIn[inputIndex].Witness = wire.TxWitness([][]byte{append(signature.Serialize(), byte(txscript.SigHashAll)), pubKey, sigScript})
					continue
				}
			}
			...
			tx.msgTx.TxIn[inputIndex].SignatureScript, err = builder.Script()
			if err != nil {
				return err
			}
			inputIndex++
